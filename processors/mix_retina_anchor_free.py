...
class DDPApexProcessor(object):
    ...
    def train(self, epoch):
        ...
        for i, (img_tensor, targets_tensor, _) in enumerate(pbar):
            ...
            loss_neg, loss_pos = detail_loss
            loss_list[0].append(total_loss.item())
            loss_list[1].append(loss_neg.item())
            loss_list[2].append(loss_pos.item())
            if self.local_rank == 0:
                pbar.set_description(
                    "epoch:{:2d}|match_num:{:4d}|size:{:3d}|total_loss:{:6.4f}|neg_loss:{:6.4f}|pos_loss:{:6.4f}|lr:{:8.6f}".format(
                        epoch + 1,
                        total_num,
                        h,
                        total_loss.item(),
            ...
            if self.tb_writer:
                tags = ['train/neg_loss', 'train/pos_loss', 'train/total_loss', 'x/lr0']  # params
                for x, tag in zip([loss_neg.item(), loss_pos.item(), total_loss.item(), lr],
                                  tags):
                    self.tb_writer.add_scalar(tag, x, i + 1 + epoch * len(self.tloader))
        self.ema.update_attr(self.model)
        mean_loss_list = [np.array(item).mean() for item in loss_list]
        print(
            "epoch:{:3d}|match_num:{:4d}|local:{:3d}|total_loss:{:6.4f}|neg_loss:{:6.4f}|pos_loss:{:6.4f}|lr:{:8.6f}"
                .format(epoch + 1,
                        match_num,
                        self.local_rank,
                        mean_loss_list[0],
